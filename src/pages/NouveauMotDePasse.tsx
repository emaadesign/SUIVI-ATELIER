...
export default function NouveauMotDePasse() {
  ...
  async function valider(e: React.FormEvent) {
    ...
    if (error) {
      setErreur("Impossible de mettre à jour le mot de passe. Redemande un nouveau lien depuis l'écran de connexion.")
    } else {
      setSucces(true)
    }
  }
  ...
  if (succes) {
    return (
      <div className="min-h-screen flex flex-col justify-center px-6 bg-linen text-center">
        ...
        <h1 className="font-display text-2xl text-plum mb-2">Mot de passe mis à jour</h1>
    ...
    <div className="min-h-screen flex flex-col justify-center px-6 bg-linen">
      ...
      <form onSubmit={valider} className="card space-y-3">
        ...
        <button className="btn-primary w-full" disabled={chargement}>
          {chargement ? 'Mise à jour...' : 'Valider le nouveau mot de passe'}
        </button>
      </form>
    </div>
  )
